...
class MPM_Solver:
    ...
    def initialize(self, n_particles, n_grid=100, grid_lim=1.0, device="cuda:0"):
        ...
        # material is used to switch between different elastoplastic models. 0 is jelly (elastic), 1 is metal (von Mises)
        self.mpm_model.material = 0

        self.mpm_model.yield_stress = wp.zeros(
            shape=n_particles, dtype=float, device=device
        )
        # hardening parameters for von Mises plasticity (material=1)
    ...
    def p2g2p(self, step, dt, device="cuda:0"):
        ...
        #### CFL check (report only) ####
        try:
            particle_v = self.mpm_state.particle_v.numpy()
            vmax = float(np.max(np.abs(particle_v)))
            dx = float(self.mpm_model.dx)
            if vmax * dt > dx:
                max_dt = dx / vmax if vmax > 1e-12 else dt
                print(
                    f"[CFL] Violated at step {step}: v_max={vmax:.4e}, dt={dt:.4e}, dx={dx:.4e}, v_max*dt={vmax*dt:.4e} > dx. Suggested dt<= {max_dt:.4e}")
        except Exception:
            pass
        #### CFL check (report only) ####
        ...
        def collide(
            ...
            if time >= param.start_time and time < param.end_time:
                ...
                if dotproduct < 0.0:
                    ...
                    elif param.surface_type == 1:  # slip
